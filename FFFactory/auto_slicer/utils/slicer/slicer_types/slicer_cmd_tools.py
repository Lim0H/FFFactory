...
class SlicerCmdParserBase(ABC):
    @abstractmethod
    def parse(self, result: str, perf_counter: float, out_dir: str) -> Any:
...
class SlicerCmdRunnerBase(ABC):
    ...
    def run(self, cmd: list[str], out_dir: str) -> tuple[Any, float]:
        perf_counter = time.perf_counter()
        result = self._run_cmd(cmd)
        return self.cmd_parser.parse(result, perf_counter, out_dir)

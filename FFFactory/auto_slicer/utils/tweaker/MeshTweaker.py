...
class Tweak(object):
    ...
    def target_function(self, bottom: float, overhang: float, contour: float, min_volume: float):
        ...
        if min_volume:  # minimize the volume of support material
            ...
            return (
                self.TAR_A * (overhang + self.TAR_B) + self.RELATIVE_F * (overhang + self.TAR_C) /
                (
    ...
    def preprocess(self, content: np.array):
    ...
    def add_supplements() -> list[list]:
        ...
        return [[list([float(j) for j in i]), 0] for i in v]

    @staticmethod
    def remove_duplicates(old_orients: list):
        """
    ...
    def project_vertices(self, orientation: np.array):
    ...
    def calc_overhang(self, orientation: np.array, min_volume: bool):
    ...
    def euler(self, bestside: np.array) -> tuple[np.array, float, np.array]:

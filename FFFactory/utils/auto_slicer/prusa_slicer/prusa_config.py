...
from FFFactory.utils.systems_util import ExistsDirType, ExistsFileType, ExistsPathType
...
from .prusa_options import (
    PrusaActionsOptionsBase,
    PrusaCommandsOptionsBase,
    PrusaSelfClearingCommandsOptionsBase,
...
from .prusa_types import LogLevelEnum


class DctPrusaGroupOptions(TypedDict):
    other_options: 'PrusaOtherOptions'
    misc_options: 'PrusaMiscOptions'
    advanced_options: 'PrusaAdvancedOptions'
    extruders_options: 'PrusaExtrudersOptions'
    extrusion_width_options: 'PrusaExtrusionWidthOptions'
    fuzzy_skin_options: 'PrusaFuzzySkinOptions'
    infill_options: 'PrusaInfillOptions'
    ironinig_options: 'PrusaIroningOptions'
    layers_and_perimeters_options: 'PrusaLayersAndPerimetersOptions'
    machine_limits_options: 'PrusaMachineLimitsOptions'
    skirt_and_brim_options: 'PrusaSkirtAndBrimOptions'
    speed_options: 'PrusaSpeedOptions'
    support_material_options: 'PrusaSupportMaterialOptions'
    wipe_options: 'PrusaWipeOptions'


class PrusaSlicerConfig(SlicerConfigBase):
    def __init__(
        self,
        slicer_path: str,
        config_path: Optional[str] = None,
        **kwargs: PrusaCommandsOptionsBase | PrusaSelfClearingCommandsOptionsBase
    ):
        super().__init__(slicer_path, config_path)
        self._actions_options = PrusaActionsOptions()
        self._transform_options = PrusaTransformOptions()
        self._files_options = PrusaFileOptions()
        if kwargs:
            group_options = DctPrusaGroupOptions(**kwargs)  # type: ignore
        else:
            group_options: DctPrusaGroupOptions = DctPrusaGroupOptions(
                other_options=PrusaOtherOptions(),
                misc_options=PrusaMiscOptions(),
                advanced_options=PrusaAdvancedOptions(),
                extruders_options=PrusaExtrudersOptions(),
        ...
        self._sliced_options = group_options
        if self.config_path is not None:
            self.sliced_options['other_options'].load = ExistsFileType(self.config_path)

    @property
    def actions_options(self) -> 'PrusaActionsOptions':
        return self._actions_options

    @property
    def transform_options(self) -> 'PrusaTransformOptions':
        return self._transform_options

    @property
    def files_options(self) -> 'PrusaFileOptions':
        return self._files_options

    @property
    def sliced_options(self) -> DctPrusaGroupOptions:
        return self._sliced_options.copy()

    @property
    def all_options(self) -> list[str]:
        lst_group_options: list[PrusaSlicedOptionsBase | PrusaSelfClearingCommandsOptionsBase] = [
            self.actions_options,
            self.transform_options,
            *self.sliced_options.values(),  # type: ignore
            self.files_options
        ]
        result: list[str] = [self.slicer_path]
        for group_option in sorted(lst_group_options, key=lambda x: x.priority.value):
            result.extend(group_option.all_options)
        return result


class PrusaActionsOptions(PrusaActionsOptionsBase):
    #  Export the model(s) as 3MF.
    export_3mf: Optional[FlagOptionType] = None
    #  Export the model(s) as AMF.
    export_amf: Optional[FlagOptionType] = None
    #  Slice the model and export toolpaths as G-code.
    export_gcode: Optional[FlagOptionType] = None
    #  Export the model(s) as OBJ.
    export_obj: Optional[FlagOptionType] = None
    #  Slice the model and export SLA printing layers as PNG.
    export_sla: Optional[FlagOptionType] = None
    #  Export the model(s) as STL.
    export_stl: Optional[FlagOptionType] = None
    #  Write information about the model to the console.
    info: Optional[FlagOptionType] = None
    #  Save configuration to the specified file.
    save: Optional[ExistsDirType] = None


class PrusaTransformOptions(PrusaTransformOptionsBase):
    """
    ...
    align_xy: Optional[XYOptionsType] = None
    center: Optional[XYOptionsType] = None
    cut: Optional[XOptionType] = None
    delete_after_load: Optional[ExistsPathType] = None
    dont_arrange: Optional[FlagOptionType] = None
    duplicate: Optional[UnsignedNOptionType] = None
    duplicate_grid: Optional[UnsignedXYOptionsType] = None
    ensure_on_bed: Optional[FlagOptionType] = None
    no_ensure_on_bed: Optional[FlagOptionType] = None
    merge: Optional[FlagOptionType] = None
    repair: Optional[FlagOptionType] = None
    rotate: Optional[XOptionType] = None
    rotate_x: Optional[XOptionType] = None
    rotate_y: Optional[XOptionType] = None
    scale: Optional[UnsignedXOptionType] = None
    scale_to_fit: Optional[UnsignedXYZOptionsType] = None
    split: Optional[FlagOptionType] = None


class PrusaOtherOptions(PrusaOtherOptionsBase):
    #  This version of PrusaSlicer may not understand configurations produced by the newest
    #  PrusaSlicer versions. For example, newer PrusaSlicer may extend the list of supported
    #  firmware flavors. One may decide to bail out or to substitute an unknown value with a
    #  default silently or verbosely. (disable, enable, enable_silent; default: enable)
    config_compatibility: Optional[FlagOptionType] = None
    ...
    datadir = ExistsDirType
    #  Do not fail if a file supplied to --load does not exist.
    ignore_nonexistent_config: Optional[FlagOptionType] = None
    #  Load configuration from the specified file. It can be used more than once to load options
    #  from multiple files.
    load: Optional[ExistsFileType] = None
    #  Sets logging sensitivity. 0:fatal, 1:error, 2:warning, 3:info, 4:debug, 5:trace
    #  For example. loglevel=2 logs fatal, error and warning level messages.
    loglevel: LogLevelEnum = LogLevelEnum.FATAL
    #  The file where the output will be written (if not specified, it will be based on the input file).
    output: Optional[ExistsPathType] = None
    #  If enabled, the command line arguments are sent to an existing instance of GUI
    #  PrusaSlicer, or an existing PrusaSlicer window is activated.
    #  Overrides the "single_instance" configuration value from application preferences.
    single_instance: Optional[FlagOptionType] = None
    #  Set the maximum number of threads the slicing process will use.
    #  If not defined, it will be decided automatically.
    threads: UnsignedNOptionType = UnsignedNOptionType(16)


class PrusaMiscOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaAdvancedOptions(PrusaSlicedOptionsBase):
    """
    --bridge-flow-ratio N
...
class PrusaExtrudersOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaExtrusionWidthOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaFuzzySkinOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaInfillOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaIroningOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaLayersAndPerimetersOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaMachineLimitsOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaSkirtAndBrimOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaSpeedOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaSupportMaterialOptions(PrusaSlicedOptionsBase):
    """
...
class PrusaWipeOptions(PrusaSlicedOptionsBase):
    """

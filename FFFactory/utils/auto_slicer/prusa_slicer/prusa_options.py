...
class SerializerOptions(object):
    ...
    def __init__(self) -> None:
        self._SERIALIZE_OPTIONS = {
            ...
            FileOptionType: lambda k, v: [self._replace_option_value(v)],
            ExistsDirNameType: lambda k, v: [self._replace_option_name(k), self._replace_option_value(v)],
            ExistsDirType: lambda k, v: [self._replace_option_name(k), self._replace_option_value(v)],
            ExistsFileType: lambda k, v: [self._replace_option_name(k), self._replace_option_value(v)],
            str: lambda k, v: [self._replace_option_name(k), self._replace_option_value(v)],
            NoneType: lambda k, v: [],
        }
    ...
    def _replace_option_value(self, option_value: str) -> str:
...
class PrusaFileOptions(PrusaSelfClearingCommandsOptionsBase):
    ...
    def __init__(self):
        super().__init__()
        self.__lst_files: list[FileOptionType] = []

    def _clear_attr(self) -> None:
        self.__lst_files = []

    def add_files(self, *files: str) -> None:
        self.__lst_files.extend(list(map(FileOptionType, files)))

    @property
    def all_options(self) -> list[str]:
        return self._get_serialized_options(dict((v, v) for v in self.__lst_files))
